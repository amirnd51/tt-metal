...
TEST_F(ProgramSpecTestGen1, DFBIncompleteCoveragePrioritizesDuplicateProducerOverMissingConsumer) {
    const NodeCoord duplicate_node{0, 0};
    const NodeCoord complete_node{1, 0};

    ProgramSpec spec;
    spec.name = "duplicate_producer_on_padding_node";
    auto producer1 = MakeMinimalGen1DMKernel("producer1", DataMovementProcessor::RISCV_0);
    auto producer2 = MakeMinimalGen1DMKernel("producer2", DataMovementProcessor::RISCV_1);
    auto consumer = MakeMinimalGen1ComputeKernel("consumer");
    auto dfb = MakeMinimalDFB("dfb");
    ...
    spec.dataflow_buffers = {dfb};
    spec.work_units = {
        MakeMinimalWorkUnit("duplicate_producers", duplicate_node, {"producer1", "producer2"}),
        MakeMinimalWorkUnit("complete", complete_node, {"producer1", "consumer"}),
    };
...
TEST_F(ProgramSpecTestGen1, DFBIncompleteEndpointCoverageRejectsDisjointRoles) {
    const NodeCoord producer_node{0, 0};
    const NodeCoord consumer_node{1, 0};

    ProgramSpec spec;
    spec.name = "disjoint_incomplete_endpoint_coverage";
    auto producer = MakeMinimalGen1DMKernel("producer", DataMovementProcessor::RISCV_0);
    auto consumer = MakeMinimalGen1ComputeKernel("consumer");
    auto dfb = MakeMinimalDFB("dfb");
    dfb.data_format_metadata = tt::DataFormat::Float16_b;
    ...
    spec.dataflow_buffers = {dfb};
    spec.work_units = {
        MakeMinimalWorkUnit("producer_only", producer_node, {"producer"}),
        MakeMinimalWorkUnit("consumer_only", consumer_node, {"consumer"}),
    };

    EXPECT_THAT(
        [&] { MakeProgramFromSpec(*mesh_device_, spec); },
        ::testing::ThrowsMessage<std::runtime_error>(
            ::testing::HasSubstr("no node has both one producer and one consumer")));
}
...
TEST_F(ProgramSpecTestGen1, DFBIncompleteEndpointCoverageFlagAcceptsCompleteTopology) {
    ProgramSpec spec = MakeMinimalGen1ValidProgramSpec();
    spec.dataflow_buffers[0].advanced_options.allow_incomplete_endpoint_coverage = true;
