...
ttnn::device_operation::ProgramArtifacts SliceWriteRMInterleavedProgramFactory::create_program_artifacts(
    ...
    for (const auto& [reader_args, writer_args] : all_runtime_args) {
        TT_FATAL(
            reader_args[4] == writer_args[7],
            "SliceWrite reader/writer barrier batch mismatch: {} vs {}",
            reader_args[4],
            writer_args[7]);
        max_num_read_per_barrier = std::max(max_num_read_per_barrier, reader_args[4]);
    }
    TT_FATAL(max_num_read_per_barrier != 0, "SliceWrite requires non-empty work");
    ...
    spec.dataflow_buffers = {DataflowBufferSpec{
        ...
        .num_entries = 2 * max_num_read_per_barrier,
    ...
    if (last_dim_strided) {
        ...
        spec.scratchpads = {ScratchpadSpec{
            ...
            .size_per_node = max_num_read_per_barrier * tt::round_up(output_row_size_bytes, alignment)}};
    }
