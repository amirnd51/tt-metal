...
ttnn::device_operation::ProgramArtifacts PaddedSliceTileProgramFactory::create_program_artifacts(
    ...
    const KernelSpec reader{
        ...
        .hw_config = ttnn::create_reader_datamovement_config(a.device()->arch(), true),
        ...
        .runtime_arg_schema =
            {.runtime_arg_names =
                 {"total_num_tiles",
                  ...
                  "misalignment"}},
        .hw_config = ttnn::create_writer_datamovement_config(a.device()->arch(), true),
        .advanced_options = {.num_runtime_varargs = 3 * rank},
    };
    ...
    for (uint32_t i = 0; i < iter_cores.size(); ++i) {
        ...
        AddRuntimeArgsForNode(
            ...
            {{"num_dims", r[1]},
             {"start_id", r[2]},
             {"num_tiles_per_core", r[3]},
             {"num_tiles_per_barrier", r[4]},
             {"num_tiles_per_row_this_core", r[5]}});
        reader_run.advanced_options.runtime_varargs.emplace(
            core, AdvancedKernelRunArgs::Varargs(r.begin() + 6, r.end()));
