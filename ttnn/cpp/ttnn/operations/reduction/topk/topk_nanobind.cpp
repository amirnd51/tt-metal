    ...
    const auto* doc =
        R"doc(
            ...
            Keyword Args:
                ...
                indices_tensor (ttnn.Tensor, optional): Input tensor containing pre-computed index values. When provided, the operation reads indices from this tensor instead of generating them. Must match the input's logical shape, be TILE layout, and is only supported when :attr:`dim` is the last dimension. A 32-bit tensor widens the output index dtype when the indices output is not preallocated; otherwise it must have the same element width as the output indices dtype. Defaults to `None`.
            ...
            Note:
                ...
                The :attr:`output_index_tensor` will be in TILE layout. Its data type is UINT16 or UINT32 by default
                (UINT16 when the tile-padded reduced dimension fits in 16 bits and the input is not FLOAT32, else UINT32).
                A 32-bit :attr:`indices_tensor` widens that default when the indices output is not preallocated.
                A preallocated index tensor pins the dtype (UINT16, UINT32, or INT32) and may be wider than the default.
