...
tt::tt_metal::DataType required_index_dtype(const ttnn::Tensor& input_tensor, int8_t dim) {
    ...
    const bool dim_fits_uint16 = input_shape[dim] <= std::numeric_limits<uint16_t>::max();
    const bool fp32_requires_uint32 = input_tensor.dtype() == tt::tt_metal::DataType::FLOAT32;
    if (dim_fits_uint16 && !fp32_requires_uint32) {
